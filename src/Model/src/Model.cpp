...
{
    auto flags = 0x0u;
    const auto add = [&flags](const aiPostProcessSteps &flag) { flags = flags | flag; };
...
std::tuple<Vertices, Normals, Indices, Meshes> make_meshes(const aiScene *const scene)
{
    ...
    static_assert(std::is_same_v<std::remove_pointer_t<decltype(aiFace::mIndices)>,
    ...
    for (const auto mesh : scene_meshes)
    {
        ...
        const auto mesh_vertices =
            std::span{reinterpret_cast<glm::vec3 *>(mesh->mVertices), mesh->mNumVertices};
        ...
        if (mesh->mNormals != nullptr)
        {
            const auto mesh_normals =
                std::span{reinterpret_cast<glm::vec3 *>(mesh->mNormals), mesh->mNumVertices};
    ...
    {
        std::cerr << "unsupported model format. all meshes must have vertex or no one. fallback - "
                     "remove normals";
        normals.clear();
    }
    return {std::move(vertices), std::move(normals), std::move(indices), std::move(meshes)};
}
...
Textures make_textures(const aiScene *const scene)
{
    ...
    for (const auto texture : std::span{scene->mTextures, scene->mNumTextures})
    {
        ...
        const auto texture_data =
            std::span{reinterpret_cast<Texel *>(texture->pcData), width * height};
...
{
    static_assert(std::is_same_v<decltype(Node::mesh_indices)::value_type,
    ...
    traverse(
        [&nodes, &num_children](const aiNode *const node) {
...
Model::Model(const fs::path &model_path, const ModelImportSettings &settings)
{
    ...
    const auto scene =
        model_importer.ReadFile(model_path.string(), import_settings_to_assimp_flags(settings));

    if (scene == nullptr)
    {
        throw std::runtime_error(fmt::format("Import model '{}' error: {}",
                                             model_path.string(),
